...
<div class="content_body">
    <div class="locate"><span class="ico_locate"></span>您当前的位置：<span><a>发送统计</a>&nbsp;>&nbsp;状态查询</span>
    </div>

    <div class="content_wrapper">
        <form action="taskDetail.html" method="post">
            <div class="content">
                <div class="search_wrap">
					...
					<label>签名:</label>
                    <input name="sign" type="text" id="sign" value="${taskDetailInfo.sign}" maxlength="20"/>
                            ...
                            <c:when test="${not empty taskDetailList}">
                                <c:forEach items="${taskDetailList}" var="taskDetail" varStatus="vs">
                                    <tr class="main_info" id="tr${taskDetail.id}">
                                        ...
                                        <c:choose>
                                            <c:when test="${taskDetail.result==0}">
                                                <td>到达成功</td>
                                                <td>${taskDetail.stat_desc}</td>
                                                <td><fmt:formatDate value="${taskDetail.arrive_time}" type="both"
                                                                    pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                            </c:when>
                                            <c:when test="${taskDetail.result==null}">
                                                <td colspan="3">未收到回执</td>
                                            </c:when>
                                            <c:otherwise>
                                                <td>到达失败</td>
                                                <td width="100px">${taskDetail.stat_desc}</td>
...
<script type="text/javascript">
    ...
    function exportSpAccount() {
        if (confirm("确认导出")) {
            ...
            $.ajax({
	            url: "<%=path%>/downLoad/validDirectDown.html?createtime=" + createtime + "&mdn=" + mdn + "&taskid=" + taskid + "&endtime=" + endtime,
	            type: "GET",
            	success: function (data) {
                	if(!(data.result)=="0"){
